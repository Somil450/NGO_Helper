...
class SupplierAuthUIFlowScreen extends StatefulWidget {
  final VoidCallback onBackToLanding;
  final VoidCallback onComplete;
  final VoidCallback onSwitchRole;

  const SupplierAuthUIFlowScreen({
    super.key,
  ...
  @override
  State<SupplierAuthUIFlowScreen> createState() => _SupplierAuthUIFlowScreenState();
}
...
class _SupplierAuthUIFlowScreenState extends State<SupplierAuthUIFlowScreen> {
  ...
  void _onBack() {
    switch (_step) {
      ...
      case _AuthStep.signup:
        ...
        break;
      case _AuthStep.supplierDetails:
        _goTo(_AuthStep.signup);
        break;
      case _AuthStep.verifyOtp:
        _goTo(_AuthStep.supplierDetails);
        break;
    }
  }
    ...
    switch (_step) {
      ...
      case _AuthStep.signup:
        return _SignUpView(
          ...
          onSupplierDetailsTap: (email, password) {
            _email = email;
            _password = password;
            _goTo(_AuthStep.supplierDetails);
          },
      ...
      case _AuthStep.supplierDetails:
        return _SupplierDetailsView(
          key: const ValueKey('supplierDetailsView'),
      ...
      case _AuthStep.verifyOtp:
        return _OTPView(
          ...
          phone: _phone,
          onBack: () => _goTo(_AuthStep.supplierDetails),
      ...
      child: GlassContainer(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Log in to your Donor account to manage your surplus tracking.',
              textAlign: TextAlign.center,
            ...
            GlassRoleToggle(isNgo: false, onSwitchRole: widget.onSwitchRole)
            ...
            GlowingButton(
              ...
              onTap: () async {
                ...
                final success = await authProvider.signInWithEmail(email: email, password: password, expectedUserType: 'supplier');
                if (success && mounted) {
            ...
            GlassSocialLogins(
               onGoogleTap: () async {
                  final auth = context.read<AuthProvider>();
                  final success = await auth.signInWithGoogle(userType: 'supplier', isLogin: true);
                  if (success && mounted) {
                    if (auth.isOnboarded) {
                       widget.onLoginSuccess();
                    }
                    ...
                    if (auth.error == 'REQUIRES_PASSWORD') {
                      _showLinkAccountDialog(context, auth, 'supplier', widget.onLoginSuccess);
                    } else {
...
class _SignUpView extends StatefulWidget {
  ...
  final void Function(String, String) onSupplierDetailsTap;
  final VoidCallback onLoginSuccess;

  const _SignUpView({
    super.key,
    required this.onLoginTap,
    required this.onSupplierDetailsTap,
    required this.onLoginSuccess,
  });
        ...
        child: Column(
          ...
          children: [
            ...
            GlowingButton(
              ...
              onTap: () {
                 ...
                 if (email.isNotEmpty && password.isNotEmpty && password == _confirmPasswordController.text) {
                   widget.onSupplierDetailsTap(email, password);
            ...
            GlassSocialLogins(
              onGoogleTap: () async {
                 final auth = context.read<AuthProvider>();
                 final success = await auth.signInWithGoogle(userType: 'supplier', isLogin: false);
                 if (success && mounted) {
                   if (auth.isOnboarded) {
                     widget.onLoginSuccess();
                   } else {
                     widget.onSupplierDetailsTap('', '');
                   }
                 } else if (auth.error != null && mounted) {
                    if (auth.error == 'REQUIRES_PASSWORD') {
                      _showLinkAccountDialog(context, auth, 'supplier', widget.onLoginSuccess);
...
class _SupplierDetailsView extends StatefulWidget {
  final VoidCallback onBack;
  final void Function(String phone, Map<String, dynamic> details) onComplete;
  
  const _SupplierDetailsView({super.key, required this.onBack, required this.onComplete});
  
  @override
  State<_SupplierDetailsView> createState() => _SupplierDetailsViewState();
}

class _SupplierDetailsViewState extends State<_SupplierDetailsView> {
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text('Donor Details', textAlign: TextAlign.center, style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.white)),
                ),
            ...
            const SizedBox(height: 24),
            const GlassLabel(text: 'Business / Individual Name'),
            GlassInputBox(hintText: 'Enter your name', controller: _nameCtrl),
            ...
            const SizedBox(height: 16),
            const GlassLabel(text: 'Food Types Usually Donated'),
            GlassInputBox(hintText: 'e.g. Cooked meals, Raw veggies...', controller: _foodCtrl),
            const SizedBox(height: 24),
            GlowingButton(
